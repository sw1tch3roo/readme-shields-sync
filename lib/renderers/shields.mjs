...
export function ghStars(ownerRepo) {
    ...
    return badge(
        ...
        img(`https://img.shields.io/github/stars/${ownerRepo}?style=social`, 'github stars'),
    );
}
...
export function ghCI(ownerRepo, workflow = 'ci.yml') {
    ...
    return badge(
        ...
        img(`https://img.shields.io/github/actions/workflow/status/${ownerRepo}/${workflow}?label=CI`, 'CI status'),
    );
}
...
export function npmVersion(name) {
    ...
    return badge(
        ...
        img(`https://img.shields.io/npm/v/${encoded}.svg`, 'npm version'),
    );
}
...
export function npmDownloads(name) {
    ...
    return badge(
        `https://www.npmjs.com/package/${name}`,
        img(`https://img.shields.io/npm/dm/${encoded}.svg`, 'npm downloads'),
    );
}
...
export function bundlephobiaMinzip(name) {
    ...
    return badge(
        ...
        img(`https://img.shields.io/bundlephobia/minzip/${encoded}`, 'bundle size'),
    );
}
...
export function pypiVersion(name) {
    ...
    return badge(`https://pypi.org/project/${name}/`, img(`https://img.shields.io/pypi/v/${name}`, 'PyPI version'));
}
...
export function cratesVersion(name) {
    ...
    return badge(`https://crates.io/crates/${name}`, img(`https://img.shields.io/crates/v/${name}`, 'crates version'));
}
...
export function goPkg(modulePath) {
    ...
    return badge(`https://pkg.go.dev/${modulePath}`, img('https://img.shields.io/badge/go-pkg.go.dev-blue', 'go pkg'));
}
...
export function coverageBadge(provider, ownerRepo) {
    ...
    if (provider === 'codecov') {
        return badge(
            ...
            img(`https://img.shields.io/codecov/c/github/${ownerRepo}`, 'coverage'),
        );
    }
    ...
    if (provider === 'coveralls') {
        return badge(
            ...
            img(`https://img.shields.io/coverallsCoverage/github/${ownerRepo}`, 'coverage'),
        );
    }
